...
htheorem' eqt_elim_true (p : Prop) (h : p = True) : p :=
  Hol.eqtElim (← Hol.assume (hol_tm(p = True)))

/-! ## `∧` -/

htheorem' and_intro (p q : Prop) (hp : p) (hq : q) : p ∧ q :=
  Hol.conj (← Hol.assume (hol_tm(p))) (← Hol.assume (hol_tm(q)))

htheorem' and_elim1 (p q : Prop) (h : p ∧ q) : p :=
  Hol.conjunct1 (← Hol.assume (hol_tm(p ∧ q)))

htheorem' and_elim2 (p q : Prop) (h : p ∧ q) : q :=
  Hol.conjunct2 (← Hol.assume (hol_tm(p ∧ q)))

/-! ## `→` -/

htheorem' imp_intro (p q : Prop) (hq : q) : p → q :=
  Hol.disch (hol_tm(p)) (← Hol.assume (hol_tm(q)))

htheorem' imp_elim (p q : Prop) (him : p → q) (hp : p) : q :=
...
htheorem' forall_elim {A : Type} (P : A → Prop) (x : A) (h : ∀ y : A, P y) : P x :=
  Hol.spec (hol_tm(x)) (← Hol.assume (hol_tm(∀ y : A, P y)))

/-! ## `False` / `¬` -/

htheorem' false_elim (p : Prop) (h : False) : p :=
  Hol.falsumElim (hol_tm(p)) (← Hol.assume (hol_tm(False)))

htheorem' not_intro (p : Prop) (h : p → False) : ¬ p :=
  Hol.notIntro (← Hol.assume (hol_tm(p → False)))

htheorem' not_elim (p : Prop) (hn : ¬ p) (hp : p) : False :=
  Hol.mp (← Hol.notElim (← Hol.assume (hol_tm(¬ p)))) (← Hol.assume (hol_tm(p)))

/-! ## `∨` -/

htheorem' or_intro1 (p q : Prop) (hp : p) : p ∨ q :=
  Hol.disj1 (hol_tm(q)) (← Hol.assume (hol_tm(p)))

htheorem' or_intro2 (p q : Prop) (hq : q) : p ∨ q :=
  Hol.disj2 (hol_tm(p)) (← Hol.assume (hol_tm(q)))

htheorem' or_elim (p q r : Prop) (h : p ∨ q) (hp : p → r) (hq : q → r) : r :=
  Hol.disjElim
...
htheorem' exists_intro {A : Type} (P : A → Prop) (x : A) (h : P x) : ∃ y, P y :=
  Hol.existsIntro (Ty.var "A") (hol_tm(P)) (hol_tm(x)) (← Hol.assume (hol_tm(P x)))

htheorem' exists_elim {A : Type} (P : A → Prop) (q : Prop)
    (hex : ∃ y, P y) (h : ∀ y : A, P y → q) : q :=
